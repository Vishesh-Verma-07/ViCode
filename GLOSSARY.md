...
| **Approval Rule** | The rule deciding whether a Tool Call pauses for user approval: a call pauses iff it is a `bash` call that is not a Silent Bash Call, or a file operation whose target is a Sensitive Path or lies outside the Project Root. Approval attaches to individual Tool Calls, not whole tools. Approving a resolved path also auto-approves later calls to that same path for the rest of the Turn, so each distinct path surfaces at most one explicit approval per Turn. |
...
| **Mode Tag** | The colored `[Build]`-style label plus left-edge bar rendered inside an Input Box from the Mode's Design Token color (build blue, discuss purple, plan orange), marking the active Mode. Updating it on a Tab cycle is immediate and never interrupts a running Turn. Distinct from an Inline Chip, which marks inline code inside prose. |
| **Inline Chip** | A short code snippet inside prose (single-backtick `` `cmd` ``) rendered with a dark background and no border, so it reads as embedded code without a full frame. Distinct from the Mode Tag, which marks the active Mode inside the Input Box. |
